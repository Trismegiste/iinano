...
class SocialControllerTest extends WebTestCasePlus
{
    ...
    public function testLikeSomebody()
    {
        ...
        $this->assertEquals(0, (int) $crawler->filter('.netizen a.fan-count')->text());
        $link = $crawler->filter('.netizen a.fan-count')->link();
        $crawler = $this->client->click($link);
        $this->assertEquals(1, (int) $crawler->filter('.netizen a.fan-count')->text());
    }
    ...
    {
        $crawler = $this->getPage('wall_index', ['wallNick' => 'spock', 'wallFilter' => 'self']);
        $link = $crawler->filter('.netizen')->selectLink('Follow')->link();
        $crawler = $this->client->click($link);
    ...
    {
        $crawler = $this->getPage('wall_index', ['wallNick' => 'spock', 'wallFilter' => 'self']);
        $link = $crawler->filter('.netizen')->selectLink('Unfollow')->link();
        $crawler = $this->client->click($link);
